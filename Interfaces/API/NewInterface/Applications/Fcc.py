...
class Fcc(Application):
  ...
  def _applicationModule(self):
    ...
    md1.addParameter(Parameter("outputFile", "", "string", "", "", False, False,
                   "The root file generated by the application"))
  ...
  def getInputFromApp(self, application):
    """Redefinition of 
    ILCDIRAC.Interfaces.API.NewInterface.Application.Application.getInputFromApp()
    in order to save input changes. 

    Output file of previous application will replace configuration file of this application.
    getInputFromApp() links automatically app1.outputFile with app2.InputFile getting in the execution module 'FccAnalysis'.
    Set fccConfFile to True to make checkConsistency() happy.

    :return: The success of getInputFromApp()
    :rtype: DIRAC.S_OK

    """
    self.fccConfFile = True
    return super(Fcc, self).getInputFromApp(application)
...
class FccSw(Fcc):
  ...
  def _checkConsistency(self, job=None):
    ...
    self.fccExecutable = '%s/run %s' % (self.fccSwPath, exe)
  ...
  def _checkFinalConsistency(self):
    ...
    Setting True 'gaudiOptionsFile' attribute tells that the application needs to generate 
