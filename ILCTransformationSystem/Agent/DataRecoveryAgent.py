...
class DataRecoveryAgent( AgentModule ):
  ...
  def execute(self):
    ...
    for transStatus in self.transformationStatus:
      ...
      if not result['Value']:
        self.log.info('No "%s" transformations of types %s to process.' % (transStatus, ", ".join(self.transformationTypes)))
        continue
    ...
    if self.ignoreLessThan:
      for trafo in tDict:
        if int(trafo) < int(self.ignoreLessThan):
          ...
          self.log.verbose('Ignoring transformation %s ( is less than specified limit %s )' % (trafo, self.ignoreLessThan))
