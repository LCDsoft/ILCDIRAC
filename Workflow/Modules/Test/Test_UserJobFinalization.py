...
class TestUserJobFinalization( unittest.TestCase ):
  ...
  def test_application_specific_inputs( self ):
    ...
    assertEqualsImproved( self.ujf.userOutputData, [ 'something1', 'something2' ], self )
  ...
  def test_application_specific_inputs_corrupt_state( self ):
    ...
    self.ujf.workflow_commons = { 'UserOutputSE' : ' Some TestOutput Storage;and its brother', 'UserOutputPath' : 'testOutputPathWwo' }
    ...
    with patch.dict('os.environ', {}, clear=True):
      ...
      assertEqualsImproved( self.ujf.failoverTest, False, self )
      assertEqualsImproved( self.ujf.userOutputData, [ 'testentryOutputData' ], self )
      assertEqualsImproved( self.ujf.userOutputSE, [ 'Some TestOutput Storage', 'and its brother' ], self )
  ...
  def test_transfer_register_and_failover_files( self ):
    ...
    with patch('%s.random.shuffle' % MODULE_NAME) as shuffle_mock:
      result = self.ujf.transferRegisterAndFailoverFiles( transfer_mock, { 'filename1test' : { 'metadatadict1' : True, 'resolvedSE' : ( 'etc', 'privateSite4' ), 'localpath' : '/my/local/path/Testme.txt', 'lfn' : 'LFN:/ilc/mytest/LFN.txt', 'filedict' : 89546 }, 'my_other_testfile' : { 'metadatadict1' : True, 'resolvedSE' : ( 'testSite1', 'privateSite4' ), 'localpath' : '/my/local/path/Testme2.txt', 'lfn' : 'LFN:/ilc/othertest/new_lfn.lfn', 'filedict' : 475 } }, filesUploaded )
      assertDiracSucceeds( result, self )
      assertEqualsImproved( result['Value'], dict(cleanUp=False), self )
      shuffle_mock.assert_any_call( [ 'testSite1', 'testSite2', 'myTestSite', 'privateSite4', 'etc' ] )
      shuffle_mock.assert_called_with( [ 'testSite1', 'testSite2', 'myTestSite', 'privateSite4', 'etc' ])
      transfer_mock.transferAndRegisterFileFailover.assert_called_with( 'filename1test', '/my/local/path/Testme.txt', 'LFN:/ilc/mytest/LFN.txt', 'etc', [ 'testSite1', 'testSite2', 'myTestSite', 'privateSite4' ], fileCatalog='blaCatalogTestme', fileMetaDict=89546 )
      transfer_mock.transferAndRegisterFileFailover.assert_any_call( 'my_other_testfile', '/my/local/path/Testme2.txt', 'LFN:/ilc/othertest/new_lfn.lfn', 'testSite1', [ 'testSite2', 'myTestSite', 'privateSite4', 'etc' ], fileCatalog='blaCatalogTestme', fileMetaDict=475 )
  ...
  def test_transferreg_transfer_fails( self ):
    ...
    filesUploaded = []
    with patch('%s.random.shuffle' % MODULE_NAME) as shuffle_mock:
      result = self.ujf.transferRegisterAndFailoverFiles( transfer_mock, { 'filename1test' : { 'metadatadict1' : True, 'resolvedSE' : ( 'nonexistant', ) , 'localpath' : '/my/local/path/Testme.txt', 'lfn' : 'LFN:/ilc/mytest/LFN.txt', 'filedict' : 89546 } }, filesUploaded )
      assertDiracSucceeds( result, self )
      assertEqualsImproved( result['Value'], dict(cleanUp=True), self )
      shuffle_mock.assert_called_once_with( [ 'testSite1', 'testSite2', 'myTestSite', 'privateSite4', 'etc' ] )
      transfer_mock.transferAndRegisterFileFailover.assert_called_once_with( 'filename1test', '/my/local/path/Testme.txt', 'LFN:/ilc/mytest/LFN.txt', 'nonexistant', [ 'testSite1', 'testSite2', 'myTestSite', 'privateSite4', 'etc' ], fileCatalog='blaCatalogTestme', fileMetaDict=89546 )
  ...
  def test_transferreg_failoverses_empty( self ):
    ...
    filesUploaded = []
    with patch('%s.random.shuffle' % MODULE_NAME) as shuffle_mock:
      result = self.ujf.transferRegisterAndFailoverFiles( transfer_mock, { 'filename1test' : { 'metadatadict1' : True, 'resolvedSE' : ( 'justOneSite', ) , 'localpath' : '/my/local/path/Testme.txt', 'lfn' : 'LFN:/ilc/mytest/LFN.txt', 'filedict' : 89546 } }, filesUploaded )
      assertDiracSucceeds( result, self )
      assertEqualsImproved( result['Value'], dict(cleanUp=True), self )
  ...
  def test_transferreg_some_files_fail( self ):
    ...
    transfer_mock = Mock()
    def my_transfer_mock( fileName, localpath, lfn, targetSE, failoverSEs, fileCatalog = None, fileMetaDict = None ): #pylint: disable=unused-argument
      """ Return S_ERROR for the appropriate file from the dictionary and S_OK for the rest. Necssary since dictionary iteration order is random """
      return TRANSFER_AND_REGISTER_DICT[fileName]
    transfer_mock.transferAndRegisterFileFailover.side_effect = my_transfer_mock
    filesUploaded = []
    with patch('%s.random.shuffle' % MODULE_NAME) as shuffle_mock:
      result = self.ujf.transferRegisterAndFailoverFiles( transfer_mock, { 'noMoreSEs' : { 'metadatadict1' : True, 'resolvedSE' : ( 'onlyAvailableSE',  ), 'localpath' : '/doesntmatter', 'lfn' : 'doesntmattereither', 'filedict' : 32 }, 'fail_transfer' : { 'metadatadict1' : True, 'resolvedSE' : ( 'someOtherSite', ), 'localpath' : '/matterdoesnt', 'lfn' : 'neither', 'filedict' : 92 }, 'workingFile1' : { 'resolvedSE' : ( 'someOtherSite', ), 'localpath' : '/my/local/first/path', 'lfn' : 'LFN:/ilc/some/dir/file1.txt', 'filedict' : 8520 }, 'thisFileWorks.too' : { 'resolvedSE' : ( 'someOtherOtherSite', ), 'localpath' : '/dir/current/local.lfn', 'lfn' : 'LFN:/ilc/mydir/file2.ppt', 'filedict' : 98453 } }, filesUploaded )
      assertDiracSucceeds( result, self )
      assertEqualsImproved( result['Value'], dict(cleanUp=True), self )
      shuffle_mock.assert_called_with( [ 'onlyAvailableSE' ] )
      transfer_mock.transferAndRegisterFileFailover.assert_any_call( 'workingFile1', '/my/local/first/path', 'LFN:/ilc/some/dir/file1.txt', 'someOtherSite', [ 'onlyAvailableSE' ], fileCatalog='myCat', fileMetaDict=8520 )
      transfer_mock.transferAndRegisterFileFailover.assert_called_with( 'thisFileWorks.too', '/dir/current/local.lfn', 'LFN:/ilc/mydir/file2.ppt', 'someOtherOtherSite', [ 'onlyAvailableSE' ], fileCatalog='myCat', fileMetaDict=98453 )
...
class TestExecute( unittest.TestCase ):
  ...
  def test_execute( self ):
    ...
    self.ujf.workflow_commons = { 'TotalSteps' : '42', 'Request' : request_mock, 'JOB_ID' : '512', 'IS_PROD' : True, 'PRODUCTION_ID' : '98245', 'Platform' : 'myTestPlatform', 'Owner' : 'myTestOwner123RichGuy', 'VO' : 'myTestVirtualOrga', 'UserOutputSE' : 'myTestReceivingSE' }
    transfer_mock = Mock()
    ...
    assertDiracSucceeds( result, self )
    constructlfn_mock.assert_called_once_with( 512, 'myTestVirtualOrga', 'myTestOwner123RichGuy', [ 'list_of.txt', 'filenames.jar' ], 'my/User/OPPath' )
    transfer_mock = transfer_mock() # Necessary for the assumptions
    transfer_mock.transferAndRegisterFile.assert_any_call('list_of.txt', '/mycurdirTestMe/list_of.txt', '/myTestVirtualOrga/testpre/m/myTestOwner123RichGuy/my/User/OPPath/list_of.txt', [ 'myTestReceivingSE', 'CERN-DIP-4' ], fileCatalog=['FileCatalog'], fileMetaDict={'Status': 'Waiting', 'ADLER32': False, 'ChecksumType': 'ADLER32', 'Checksum': False, 'LFN': '/myTestVirtualOrga/testpre/m/myTestOwner123RichGuy/my/User/OPPath/list_of.txt', 'GUID': None, 'Addler': False, 'Size': 3048})
    transfer_mock.transferAndRegisterFile.assert_called_with('filenames.jar', '/mycurdirTestMe/filenames.jar', '/myTestVirtualOrga/testpre/m/myTestOwner123RichGuy/my/User/OPPath/filenames.jar', [ 'myTestReceivingSE', 'CERN-DIP-4' ], fileCatalog=['FileCatalog'], fileMetaDict={'Status': 'Waiting', 'ADLER32': False, 'ChecksumType': 'ADLER32', 'Checksum': False, 'LFN': '/myTestVirtualOrga/testpre/m/myTestOwner123RichGuy/my/User/OPPath/filenames.jar', 'GUID': None, 'Addler': False, 'Size': 3048})
  ...
  def test_execute_getcandidatefiles_fails( self ):
    self.ujf.workflow_commons = { 'TotalSteps' : '42', 'Owner' : 'something', 'VO' : 'something_else' }
  ...
  def test_execute_constructlfn_fails( self ):
    self.ujf.workflow_commons = { 'TotalSteps' : '42', 'JOB_ID' : '512', 'IS_PROD' : True, 'PRODUCTION_ID' : '98245', 'Platform' : 'myTestPlatform', 'Owner' : 'myTestOwner123RichGuy', 'VO' : 'myTestVirtualOrga' }
    self.ujf.userOutputData = [ 'list_of.txt', 'filenames.jar' ]
  ...
  def test_execute_ignoreapperrors( self ):
    ...
    with patch('%s.constructUserLFNs' % MODULE_NAME, new=Mock(return_value=S_OK(['/myTestVirtualOrga/testpre/m/myTestOwner123RichGuy/my/User/OPPath/list_of.txt', '/myTestVirtualOrga/testpre/m/myTestOwner123RichGuy/my/User/OPPath/filenames.jar']))) as constructlfn_mock, \
      ...
      result = self.ujf.execute()
    assertDiracSucceeds( result, self )
    getcf_mock.assert_called_once_with( [{'outputPath': 'TXT', 'outputFile': 'list_of.txt', 'outputDataSE': ['myTestReceivingSE']}, {'outputPath': 'JAR', 'outputFile': 'filenames.jar', 'outputDataSE': ['myTestReceivingSE']}], ['/myTestVirtualOrga/testpre/m/myTestOwner123RichGuy/my/User/OPPath/list_of.txt', '/myTestVirtualOrga/testpre/m/myTestOwner123RichGuy/my/User/OPPath/filenames.jar'], '' )
    constructlfn_mock.assert_called_once_with( 512, 'myTestVirtualOrga', 'myTestOwner123RichGuy', [ 'list_of.txt', 'filenames.jar' ], 'my/User/OPPath' )
    getfmd_mock.assert_called_once_with( {} )
    transfer_mock = transfer_mock() # Necessary for the assumptions
    transfer_mock.transferAndRegisterFile.assert_any_call( 'workingFile1', '/my/local/first/path', 'LFN:/ilc/some/dir/file1.txt', ['myTestReceivingSE', 'CERN-DIP-4'], fileCatalog=['FileCatalog'], fileMetaDict=8520 )
    transfer_mock.transferAndRegisterFile.assert_called_with( 'thisFileWorks.too', '/dir/current/local.lfn', 'LFN:/ilc/mydir/file2.ppt', ['myTestReceivingSE', 'CERN-DIP-4'], fileCatalog=['FileCatalog'], fileMetaDict=98453 )
    assertEqualsImproved( len(transfer_mock.transferAndRegisterFile.mock_calls), 14, self )
    self.assertFalse( transfer_mock.transferAndRegisterFileFailover.called )
    dataman_mock = dataman_mock()
    dataman_mock.replicateAndRegister.assert_any_call( 'LFN:/ilc/mydir/file2.ppt', 'myTestReceivingSE')
    dataman_mock.replicateAndRegister.assert_called_with( 'LFN:/ilc/some/dir/file1.txt', 'myTestReceivingSE' )
