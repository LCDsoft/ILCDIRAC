...
class FccAnalysis(ModuleBase):
  ...
  def runIt(self):
    ...
    if self.InputFile and not self.isGaudiOptionsFileNeeded:
      self.SteeringFile = " ".join(self.InputFile) if isinstance(self.InputFile, list) else self.InputFile
    ...
    # Main command
    bashCommands = ['%s %s %s' % (self.fccExecutable, self.SteeringFile, self.gaudiOptionsFile)]
    ...
    if not rootFiles:
      ...
      # and not generated by the previous ones
      ...
      if not currentRootFiles:
        ...
        if not self.OutputFile:
          appIndex = "%s_%s_%s" % (self.applicationName, self.applicationVersion, self.STEP_NUMBER)
          self.OutputFile = ["%s_%s" % (appIndex, currentRootFile) for currentRootFile in currentRootFiles]

        outputFiles = [self.OutputFile] if isinstance(self.OutputFile, str) else self.OutputFile

        # if the user expected the exact number of root files, it is ok
        # else do not care, we can not guess for him

        if len(outputFiles) == len(currentRootFiles) :

          for outputFile, currentRootFile in zip(outputFiles, currentRootFiles):

            old = os.path.realpath(currentRootFile)
            ...
            # at the user level through Job.setOutputData("myOutput.root") because we have renamed it
