...
class FccAnalysis(ModuleBase):
  ...
  def runIt(self):
    ...
    # Each file generated by the application is indexed by 'fccAppIndex'
    ...
    if not rootFiles:
      ...
      if not currentRootFiles:
        ...
        outputFiles = ["%s_%s.root" % (os.path.splitext(currentRootFile)[0], self.fccAppIndex) for currentRootFile in currentRootFiles]
        ...
        for outputFile, currentRootFile in zip(outputFiles, currentRootFiles):
          ...
          # Rename last root file(s) generated by the application
          ...
          # at the user level through Job.setOutputData("myOutput.root") because we have renamed it just before !
          if 'UserOutputData' in self.workflow_commons:
            ...
            for idx, data in enumerate(outputData):
              if data.endswith(".root") and old == os.path.realpath(os.path.basename(data)):
                ...
                outputData[idx] = os.path.join(lfnTree, os.path.basename(renamedRootFile))
