...
def run_step(phaseID, stepID):
  """ Runs a calibration step on the worker for the given phase and step.

  :param int phaseID: the current phase
  :param int stepID: number of the current step
  :returns: nothing
  :rtype: None
  """
  phase = CalibrationPhase.phaseNameFromID(phaseID)
  method_to_run = None
  if phase == 'ECalDigi':
    method_to_run = ecal_digitisation
  elif phase == 'HCalDigi':
    method_to_run = hcal_digitisation
  elif phase == 'MuonDigi':
    method_to_run = muon_digitisation
  elif phase == 'ElectroMagEnergy':
    method_to_run = electromagnetic_energy_scale
  elif phase == 'HadronicEnergy':
    method_to_run = hadronic_energy_scale
  else:
    raise ValueError(
        'Should never happen: run_step received an invalid CalibrationPhase (usually caught in CalibrationPhase itself)')
  #mv pfoAnalysis*.root #FIXME: correct this
  return method_to_run(stepID)


WAITING_TIME_BETWEEN_POLLING = 2.0
...
def distributed_calibration():
  ...
  answer = client.requestNewParameters()  # Calibration over is S_ERROR with string in it
  # New parameter set is S_OK with histogram in it
  # No parameters available yet is S_OK with string in it
  while not answer['OK']:
    if isinstance(answer['Value'], basestring):  # no new params yet
      time.sleep(WAITING_TIME_BETWEEN_POLLING)
    else:  # Value is histogram, jump to phase and step and calculate
      client.jumpToStep(answer['current_phase'], answer['current_step'])
      result = run_step(answer['current_phase'], answer['current_step'])
      client.reportResult(result)
    answer = client.requestNewParameters()
  gLogger.info('Finished calibration run.')
