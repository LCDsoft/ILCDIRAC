...
linux_flavour_aliases = { "slc"  : ["Scientific Linux"],
...
lsb_flavour_aliases   = { "sl"   : ["ScientificSL"],
                          "slc"  : ["ScientificCERNSLC"],
                          "fc"   : ["Fedora"],
                          "co"   : ["CentOS"]
                        }

flavor_runtime_compatibility = { "slc6"  : ["slc6", "slc5"],
                                 "slc5"  : ["slc5", "slc4"],
                                 "slc4"  : ["slc4", "slc3"],
                                 "slc3"  : ["slc3"],
                                 "rh73"  : ["rh73"],
...
arch_runtime_compatiblity = { "x86_64" : ["x86_64", "i686", "i586", "i486", "i386"],
...
flavor_runtime_equivalence = { "slc6"  : ["slc6", "co6", "rhel6" ],
...
supported_compilers = { "slc6"   : ["gcc44"],
                        "slc5"   : ["gcc43"],
                        "slc4"   : ["gcc34"],
                        "slc3"   : ["gcc323"],
                        "win32"  : ["vc71"],
  ...
  def OSFlavour(self, teststring=None):
    ...
    if not self._osflavor or teststring:
      if self._ostype == "Windows" :
        ...
        matches = vmatch.search(cont)
        if matches:
          self._osversion = matches.group(1)
  ...
  def nativeCompilerVersion(self, position=None):
    ...
    if not self._compversion :
      if self._ostype == "Windows" :
        ...
        vmatch = re.compile(r"\ +(\d+(?:\.\d+)*)")
        matches = vmatch.search(compstr)
        if matches:
          self._compversion = matches.group(1)
